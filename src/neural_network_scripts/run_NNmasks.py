...
try:
        ...
        for i in range(T):# I think this unpacks all the segmented/mask frames -MB
            ...
            if from_points:
                if pts_exists[i]:
                    ...
                    Nans+=1
            elif str(i)+"/mask" in h5.keys(): #Todo-MB: add cropped or aligned version?
    ...
    print(totnum)
    tnum = int(input("traning set number")) #MB added
    vnum = int(input("validation set number"))
    if tnum=="all" or vnum==0:#vnum should exist if tnum!="all"
        ...
        '''
        MB removed since this part was absent in run_NNpoint and raised an error here
