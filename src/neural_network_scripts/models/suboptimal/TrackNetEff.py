...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate,kernel_size):
        super().__init__()
        self.conv=nn.Conv3d(num_input_features, growth_rate,kernel_size=kernel_size, stride=1, padding=2,bias=False)
...
class _Transition(nn.Module):
    def __init__(self, num_input_features, num_output_features):
        super().__init__()
        self.poolconv=nn.Conv3d(num_input_features,num_output_features,kernel_size=4, stride=2,padding=1,bias=False)
...
class Net(nn.Module):
    def __init__(self, n_channels=3,growth_rate=(48,16,16,16), block_config=(1,4,4,4),bottleneck_num_layers=3, compression=1.0,num_init_features=32,kernel_size=5,num_classes=10):

        super().__init__()
        assert 0 < compression <= 1, 'compression of densenet should be between 0 and 1'

        # First convolution
        self.conv3d0=nn.Conv3d(n_channels, num_init_features, kernel_size=5, stride=1,padding=2,bias=False)
        ...
        #for bottleneck up
        for i,(num_layers,growth_rate) in enumerate(zip(list(block_config)[::-1],list(growth_rates)[::-1])):
