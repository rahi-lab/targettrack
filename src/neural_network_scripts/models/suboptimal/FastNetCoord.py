...
class Net(nn.Module):
    def __init__(self, n_channels=3,growth=32,kernel_size=3,compress_targ=64,num_classes=10):
        ...
        n_filt_in=n_filt+3
        n_filt+=growth
        self.conv3=nn.Conv3d(n_filt_in, n_filt, kernel_size=5, stride=1,padding=2,bias=False)
        self.norm3=nn.BatchNorm3d(n_filt)
        self.conv4=nn.Conv3d(n_filt, n_filt, kernel_size=5, stride=1,padding=2,bias=False)
        ...
        self.compression5conv=nn.Conv3d(n_filt, compress_targ, kernel_size=1, stride=1,bias=False)
        self.compression5bn=nn.BatchNorm3d(compress_targ)
        self.up5=nn.Upsample(scale_factor=8)

        self.conv_out=nn.Conv3d(n_channels+growth+2*growth+3*(compress_targ//2)+compress_targ,num_classes, kernel_size=1, stride=1,bias=True)
    ...
    def forward(self, x,verbose=False):
        if not self.initialized:
            xg,yg,zg=torch.meshgrid(torch.linspace(-1.,1,x.size(2)),torch.linspace(-1.,1,x.size(3)),torch.linspace(-1,1,x.size(4)))
            self.g0=torch.cat([xg[None,...],yg[None,...],zg[None,...]],dim=0)
            fac=2
            xg,yg,zg=torch.meshgrid(torch.linspace(-1.,1,x.size(2)//fac),torch.linspace(-1.,1,x.size(3)//fac),torch.linspace(-1,1,x.size(4)//fac))
            ...
            fac=16
            #z=16 specific
            xg,yg,zg=torch.meshgrid(torch.linspace(-1.,1,x.size(2)//fac),torch.linspace(-1.,1,x.size(3)//fac),torch.ones(1))
